...
public class PersonLoader {
	...
	public PersonLoader(AffinityBrowser parent) {
		type = new ModelType();
	...
	public void load() {
		...
		Resource overview = new Resource(GWT.getHostPageBaseURL()
				+ "data/cached/discovery.json");

		overview.get().send(new JsonCallback() {
			...
			public void onSuccess(Method method, JSONValue response) {
				...
				for (String uri : uris) {
					...
					resources.add(new Resource(GWT.getHostPageBaseURL()
							+ "data/cached/profile" + adaptedUri + ".json"));
				}
				...
				for (Resource resource : resources) {
					resource.get().send(new myJsonCallback());
				}
			}
	...
	protected class myJsonCallback implements JsonCallback {
		public void onSuccess(Method method, JSONValue response) {
			try {
